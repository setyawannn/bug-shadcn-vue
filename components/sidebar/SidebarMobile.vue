<template>
  <div
    class="pt-4 border-r dark:border-zinc-600 flex flex-col flex-grow min-h-full dark:bg-zinc-900 sidebar w-80"
  >
...
.sidebar {
  position: fixed;
  top: 5rem; /* Sesuaikan dengan tinggi header Anda */
  left: 0;
  width: 15rem; /* Atau sesuaikan lebar sidebar */
  height: calc(100% - 64px); /* Kompensasi tinggi header */
  background-color: #fff;
  transform: translateX(-100%);
  transition: transform 0.3s ease-in-out;
  z-index: 200; /* Pastikan lebih tinggi dari konten utama namun di bawah z-index header jika perlu */
}
